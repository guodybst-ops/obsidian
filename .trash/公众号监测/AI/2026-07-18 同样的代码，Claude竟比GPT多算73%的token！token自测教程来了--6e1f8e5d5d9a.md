...
公众号: "Datawhale"
发布日期: 2026-07-18
抓取时间: 2026-07-19 08:49:29
类别: AI
自动检查结果: "B 存参考资料"
相关度评分: 10
命中词: ["AI"]
原始链接: "https://mp.weixin.qq.com/s/SiqsZA0R7cWh6OO_N34g1w"
文章ID: "2247724293_1"
---

# 同样的代码，Claude竟比GPT多算73%的token！token自测教程来了

#          Datawhale干货

作者：   Ruslan Ianberdin

模型的账单价格标的是"每百万token多少钱"，但没人告诉你，同一段代码在不同模型上会被切成不同数量的token。这件事直接决定了你实际付了多少钱，而且大部分人从来没测过。

Playcode本月发布的一份测评给出了具体数字：同一份2888字符的TypeScript文件，GPT用o200k分词器切出681个token，Claude最新的分词器切出1178个，多了     73%     。挂牌单价没有变化，实际花费已经变了。

这篇文章分两部分：先说清楚这个差距从哪来，再给出你自己动手测的步骤。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSic7rQiaGzuyQjNv9rPyMapTI2cCibp8JRnGEuILPIVtkColMFFCjicYE27Rx0KMeiaA6EPmp90o8ib28696bT4r7O0DoUAYoESMErDY/640?wx_fmt=png&from=appmsg)

中文这一行几乎没变化，涨幅主要集中在英文和代码上。

跨厂商对比：

![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cS8AZPMO80sGFaqCcRw8dVoPdyXfXb7NFQaKdiaTQwhNULM6viasYdTmmTeKjvvjYumMSGl5kON4TEWUHQTJwpmjibOwDBKXNA9vEQ/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSibLvBFOpvbZZTUlbLWicZ137I4eJRywF9CWBWCxvFkwLBX93seMo2dnXd0H4bWJDjtFaibu9ljlBSDQicLV08veuJAmMW7mXFzuibQ/640?wx_fmt=png&from=appmsg)
...
需要说明的是，这只是输入端的分词差异。模型回复的啰嗦程度、思考token、缓存读写频率、工具调用次数，这些变量叠加起来，实际任务总花费的差距可能远超过73%这个数字    有人反映实际用下来差2到4倍，也是合理的，只是这已经是另一层变量了。
...
import  tiktoken    enc = tiktoken.get_encoding( "o200k_base" )    tokens = enc.encode(你的文本)     print ( len (tokens))
...
- 公众号：Datawhale
- [查看微信原文](https://mp.weixin.qq.com/s/SiqsZA0R7cWh6OO_N34g1w)

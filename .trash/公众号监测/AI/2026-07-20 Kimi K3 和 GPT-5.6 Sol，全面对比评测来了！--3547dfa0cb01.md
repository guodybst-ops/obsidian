...
公众号: "Datawhale"
发布日期: 2026-07-20
抓取时间: 2026-07-20 22:50:52
类别: AI
自动检查结果: "B 存参考资料"
相关度评分: 10
命中词: ["AI"]
原始链接: "https://mp.weixin.qq.com/s/0v8CuUp3d5mGBCDzSZKcrg"
文章ID: "2247724385_1"
---
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cS9dPJlfALuvEX1muTzH5cThykKtKEHGicm5f8ehDMJJZppnfBXRjicHcibAMNcqGGPPOESXB2aK6UGtfP5z4SyTqt48s9qzTjtAJs/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/zW6S9vt0cS8JBvudicP5Vt1FBWAianJ1y6iaunb6cLDoGGhtTuurViabNgu31eScn3sC0xXSCBlOtOmFtYS6oP4aprxItPD69Dj3WUWpJzQUm7U/640?wx_fmt=jpeg&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSib5zsu90zZ59UzVxgfvLuOt8K6BJDib3MJy6U79amUCp9kf1zQ9Zvqpr0njq87X3e5XHziaHyHF96UVQHpmiaPeoDAtTyicUh4mvDY/640?wx_fmt=png&from=appmsg)

##   二、评价一个模型：从"能跑"到"能玩"
判断模型代码能力，常见的做法是让它生成单文件前端页面。这种方式成本低，但看不出复杂工程里的真实水平。

3D 游戏开发是更好的试金石：它涉及渲染、碰撞、状态管理、实时交互，模型不只要写代码，还要根据页面视觉反馈反复调试。于是我们让两个模型用      完全相同的  提示词和视觉约束      ，各自复刻 5 款童年经典游戏，比谁更能跑出可玩的成品。
...
###    1. 环境启动与依赖修复
K3 的第一步是安装依赖并启动服务。它正确识别了 FastAPI 后端需要的全部依赖并安装完成，随后执行     uvicorn main:app --host 0.0.0.0 --port 8001    启动服务。

![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cS8xoEkQrhSqrDE4liczIgvTY62Ubjp8WK9SQhUGmeA3E87YlHYVnOtSIHZ0uI5bhsnsMicqVOxFkCG5kAM54Kkhs7F5njOSFz9KI/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cSicpmGAILdcbticEl0NrEBKdLD5Ta5F4e9Sicm06891NnHbCS8JsI335Ksu4MialZQHKpzmlpDZQV85RdibgVRtUEkdPRCTgaLWEVIU/640?wx_fmt=png&from=appmsg)

###    2. 后端功能实现与接口验证
核心需求是给论文引用关系增加完整的后  端能力。K3 改动了三个文件：

•     database.py    ：新增     paper_references    表，含复合主键、外键、索引；

•     graph.py    ：新增     build_reference_graph()    和     export_reference_html()    ；

•     main.py    ：新增 4 个引用端点 + 2 个图谱端点。

![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cS8U10VqgEppubDbBpWaSINx2j8aOjGLpjdkYrC09ib2wzTyibd2WiavpPeYiaeqmsIicpP82nAgS6zKNUlfea2icorQ8mGq731PwbptU/640?wx_fmt=png&from=appmsg)

为了验证交付质量，K3 在全新端口 8002 上跑了 8 步端到端测试，全部通过：入库论文、添加引用、查询 outgoing/incoming、获取 graph、导出 HTML、删除引用、删除后验证为空。新增测试 36 个全部通过；全量 134 passed、2 failed，K3 用     git stash    撤回自己的改动后这 2 个失败依然存在，证明是项目预存的 legacy 问题，与本次功能无关。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cSibtcA6Fiao0Am0QZeqiaWbzVeibQrPYgbeVTlQLSmzB29ul3EQNic5oMVUSrUC44LasMn44CoibxvichNiaVCBmibCWP3C40hdYduatOuo/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSic1hldAQUqtwJofx0ibOAn3rs2LhSthicAkng8dC4mloKOVia1I5LgdmnGGerVJPXQRYOrwyV55nvtUibiaIeROicVU1yGt2Hlib3SzDU/640?wx_fmt=png&from=appmsg)

K3 没有卡住，而是通过 Kimi Code 的 WebBridge 直接控制浏览器验证页面状态：注入 JS 探测     rootLen    ，确认页面从 0 增长到 30081，说明前端确实渲染出来了。

![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSicqHO2qLUicXX7Y4S9x82FrwHdUiazgrdAaC2kT1S2PvQfNY2ZSPicnUMRpAVRKicXcDYxGsBTLGnBNZYQIJPorstmdssxKa8h7m74/640?wx_fmt=png&from=appmsg)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/zW6S9vt0cSibH8qrlnseEnocwXUortaCK2wrfsoKVcOtmPsHVzaAoiamWg5FfnOOMZO5LANSKTSUK6KssEUPNdmO4Gm5ofBSCQQQy2ibgdibVs8/640?wx_fmt=png&from=appmsg)

##   六、写在最后
Kimi K3 在呈现效果、游戏逻辑和操作流畅度上表现出色。作为开源模型，能达到这个完成度已是少数能打的存在，是    中国开源模型参数的里程碑    。

但就在昨天，K3 因算力不足已整体关闭购买渠道，这也说明 Kimi 比较在意老用户体验。感兴趣的小伙伴，可以等他们算力充足后再购买体验。

![图片](https://mmbiz.qpic.cn/mmbiz_png/zW6S9vt0cSibUdiaicKMCq8HaCJrcuCPuck2czxibSP3N7cNrtoribkDMforH9SuiaOjHjXBYxnMJvlw3bbwWice3FyxOSKmjulPeiccBibDg8d3PvGI/640?wx_fmt=png&from=appmsg)
...
- 公众号：Datawhale
- [查看微信原文](https://mp.weixin.qq.com/s/0v8CuUp3d5mGBCDzSZKcrg)
